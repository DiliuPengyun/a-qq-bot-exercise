...
MAX_TOOL_TURNS: int = 3            # 工具调用多轮上限（防死循环）：首次 + 纠正 + 余量
...
SYSTEM_PROMPT_BASE = (
    ...
    "调用工具时，你可以先输出 <message> 说一句（比如「我查一下」），这条会立即发给用户；工具返回后再输出最终回复。也可以不说话直接调工具。"
    "\n"
    "=== 消息格式 ==="
    "对话历史中每条消息格式为：\n"
...
CHAT_REPAIR_PROMPT = (
    "你之前的回复输出格式有误，XML 标签不匹配导致解析失败。\n"
    "请重新输出，确保：\n"
    "1. 正式回复用 <message>...</message> 包裹，可以有多条\n"
    "2. 情绪用 <mood p=\"-1~1\" a=\"-1~1\" d=\"-1~1\" reason=\"...\">标签文字</mood> 包裹\n"
    "3. 所有标签必须正确闭合\n"
    "4. 只输出修正后的内容，不要解释\n\n"
    "你之前输出的内容：\n"
)
...
MOOD_REPAIR_PROMPT = (
    "你之前的输出缺少可解析的情绪标签，或 <mood> 标签的 p/a/d 数值无法解析。\n"
    "请补上或修正 <mood> 标签，确保：\n"
    "1. 格式为 <mood p=\"-1~1\" a=\"-1~1\" d=\"-1~1\" reason=\"简短原因\">情绪标签文字</mood>\n"
    "2. p/a/d 必须是 -1 到 1 之间的数字（可带正负号和小数点）\n"
    "3. 只输出修正后的 <mood> 标签，不要输出其他内容\n\n"
    "你之前输出的内容：\n"
)
